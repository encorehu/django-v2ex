...
from v2ex.babel.models import PasswordResetToken
...
class HomeHandler(TemplateView):
    ...
    def get_tamplate_names(self):
        if self.browser['ios']:
            return ['mobile/index.html',self.template_name]
    
    def dispatch(self, request, *args, **kwargs):
        self.browser = detect(request)
        return super(HomeHandler, self).dispatch(request, *args, **kwargs)
    
#    def get_context_data(self, **kwargs):
#        context = super(HomeHandler, self).get_context_data(**kwargs)
#        site = GetSite()
#        context['site'] = GetSite()
#        context['canonical'] = 'http://' + site.domain + '/'
#        context['rnd'] = random.randrange(1, 100)
#        context['page_title'] = site.title
...
#        return context
    ...
    def get(self, request):
        ...
        if member:
            print '--------------------------------------------------yes'
            #self.response.META['Set-Cookie'] = 'auth=' + member.auth + '; expires=' + (datetime.datetime.now() + datetime.timedelta(days=365)).strftime("%a, %d-%b-%Y %H:%M:%S GMT") + '; path=/'
            response = HttpResponse() # replace redirect with HttpResponse or render
            cookie_args = { 
                'value': 'member.auth', 
                'max_age': settings.SESSION_COOKIE_AGE, 
                'expires': (datetime.datetime.now() + datetime.timedelta(days=365)).strftime("%a, %d-%b-%Y %H:%M:%S GMT"),
                'path': '/', 
                'secure': False
            }
            #response.set_cookie('cookie_name', 'cookie_value', max_age=1000)
            response.set_cookie("auth", **cookie_args) 
        ...
        if member:
            recent_nodes = cache.get('member::' + str(member.num) + '::recent_nodes')
            if recent_nodes:
                template_values['recent_nodes'] = recent_nodes
        nodes_new = []
        nodes_new = cache.get('home_nodes_new')
        if nodes_new is None:
            nodes_new = []
            
            #qnew = db.GqlQuery("SELECT * FROM Node ORDER BY created DESC LIMIT 10")
            qnew = Node.objects.order_by('-created').all()[:10]
            if (qnew.count() > 0):
                i = 0
                for node in qnew:
                    nodes_new.append(node)
                    i = i + 1
            cache.set('home_nodes_new', nodes_new, 86400)
        ...
        if browser['ios']:
            home_rendered = cache.get('home_rendered_mobile')
            if home_rendered is None:
                latest = cache.get('q_latest_16')
                if (latest):
                    template_values['latest'] = latest
                else:
                    #q2 = db.GqlQuery("SELECT * FROM Topic ORDER BY last_touched DESC LIMIT 16")
                    q2 = Topic.objects.order_by('-last_touched').all()[:16]
                    topics = []
                    for topic in q2:
                        if topic.node_name not in ignored:
                            topics.append(topic)
                    cache.set('q_latest_16', topics, 600)
                    latest = topics
                    template_values['latest'] = latest
                path = 'portion/home_mobile.html'
                home_rendered = template.loader.render_to_string(path, template_values)
                cache.set('home_rendered_mobile', home_rendered, 600)
            template_values['home'] = home_rendered
        else:
            home_rendered = cache.get('home_rendered')
            if home_rendered is None:
                latest = cache.get('q_latest_16')
                if (latest):
                    template_values['latest'] = latest
                    ...
                    latest = topics
                    template_values['latest'] = latest
                path = 'portion/home.html'
                home_rendered = template.loader.render_to_string(path, template_values)
                cache.set('home_rendered', home_rendered, 600)
            template_values['home'] = home_rendered
        member_total = cache.get('member_total')
        if member_total is None:
            #q3 = db.GqlQuery("SELECT * FROM Counter WHERE name = 'member.total'")
            q3 = Counter.objects.filter(name='member.total').all()
            if (q3.count() > 0):
                member_total = q3[0].value
            else:
                member_total = 0
            cache.set('member_total', member_total, 3600)
        ...
        if topic_total is None:
            ...
            q4 = Counter.objects.filter(name='topic.total').all()
            if (q4.count() > 0):
                topic_total = q4[0].value
            else:
                topic_total = 0
            cache.set('topic_total', topic_total, 3600)
        ...
        if reply_total is None:
            ...
            q5 = Counter.objects.filter(name='reply.total').all()
            if (q5.count() > 0):
                reply_total = q5[0].value
            else:
                reply_total = 0
            cache.set('reply_total', reply_total, 3600)
        template_values['reply_total'] = reply_total
        hottest = cache.get('index_hottest_sidebar')
        if hottest is None:
            #qhot = db.GqlQuery("SELECT * FROM Node ORDER BY topics DESC LIMIT 25")
            qhot = Node.objects.order_by('-topics').all()[:25]
            hottest = u''
            for node in qhot:
                hottest = hottest + '<a href="/go/' + node.name + '" class="item_node">' + node.title + '</a>'
            cache.set('index_hottest_sidebar', hottest, 86400)
        template_values['index_hottest_sidebar'] = hottest
        c = cache.get('index_categories')
        if c is None:
            c = ''
            i = 0
            ...
            for category in categories:
                ...
                qx = Node.objects.filter(category=category).order_by('-topics').all()
                for node in qx:
                    c = c + '<a href="/go/' + node.name + '" style="font-size: 14px;">' + node.title + '</a>&nbsp; &nbsp; '
                c = c + '</td></tr></table></div>'
                cache.set('index_categories', c, 86400)
        template_values['c'] = c
        if (browser['ios']):
            path = 'mobile/index.html'
        else:
            path = 'desktop/index.html'
        #output = template.loader.render_to_string(path, template_values)
        #return output #self.response.out.write(output)
...
class PlanesHandler(BaseHandler):
    def get(self, request):
        ...
        if (s == None):
            ...
            cache.set('planes', s, 86400)
            cache.set('planes_c', c, 86400)
        self.values['c'] = c
        self.values['s'] = s
        self.values['page_title'] = self.site.title.decode('utf-8') + u' › ' + self.l10n.planes.decode('utf-8')
        return self.finalize(template_name='planes')
        
class RecentHandler(View):
    def get(self, request):
        site = GetSite()
        browser = detect(self.request)
        template_values = {}
...
class SigninHandler(View):
    def get(self, request):
        ...
        template_values['next'] = request.META.get('HTTP_REFERER', None)

        if browser['ios']:
            path = 'mobile/signin.html'
        else:
            path = 'desktop/signin.html'
    ...
    def post(self,request):
        ...
        template_values['page_title'] = site.title + u' › 登入'
        template_values['system_version'] = SYSTEM_VERSION
        u = self.request.POST.get('u','').strip()
        p = self.request.POST.get('p','').strip()
        l10n = GetMessages(self, member, site)
        template_values['l10n'] = l10n
        errors = 0
        ...
        if (len(u) > 0 and len(p) > 0):
            ...
            if '@' in u:
                ...
                q = Member.objects.filter(email=u.lower(), password=p_sha1)
            else:
                #q = db.GqlQuery("SELECT * FROM Member WHERE username_lower = :1 AND password = :2", u.lower(), p_sha1)
                q = Member.objects.filter(username_lower=u.lower(), password=p_sha1)
            ...
            if (q.count() == 1):
                ...
                next = self.request.POST.get('next').strip()
                host = self.request.META['HTTP_HOST'] + '/'
...
class SignupHandler(View):
    def get(self, request):
        ...
        template_values['l10n'] = l10n
        if browser['ios']:
            path = 'mobile/signup.html'
        else:
            path = 'desktop/signup.html'
        #output = template.render(path, template_values)
        #self.response.out.write(output)
        return render(request, path, template_values)
    ...
    def post(self, request):
        ...
        member_username_error_messages = ['',
            ...
            l10n.username_taken]
        member_username = self.request.POST.get('username','').strip()
        # Special cases
        ...
        member_password = self.request.POST.get('password','').strip()
        ...
        if (len(member_email) == 0):
            ...
            if (len(member_email) > 32):
                ...
                if (p.search(member_email)):
                    ...
                    q = Member.objects.filter(email = member_email.lower())
                    if (q.count() > 0):
                        errors = errors + 1
...
#            template_values['recaptcha_error_message'] = '请重新输入 reCAPTCHA 验证码'
        ...
        if (errors == 0):
            ...
            #self.response.META['Set-Cookie'] = 'auth=' + member.auth + '; expires=' + (datetime.datetime.now() + datetime.timedelta(days=365)).strftime("%a, %d-%b-%Y %H:%M:%S GMT") + '; path=/'
            response = HttpResponseRedirect('/') # replace redirect with HttpResponse or render
            cookie_args = { 
                'value': 'member.auth', 
                'max_age': settings.SESSION_COOKIE_AGE, 
                'expires': (datetime.datetime.now() + datetime.timedelta(days=365)).strftime("%a, %d-%b-%Y %H:%M:%S GMT"),
...
class SignoutHandler(View):
    def get(self, request):
        ...
        #del cookies['auth']
        if browser['ios']:
            path = 'mobile/signout.html'
        else:
            path = 'desktop/signout.html'
        #output = template.render(path, template_values)
        #self.response.out.write(output)
        response = render(request, path, template_values)
    ...
    def post(self, request):
        ...
        if q.count() == 1:
            ...
            if q2.count() > 2:
                ...
                mail_template_values['host'] = self.request.META['Host']
                mail_template_values['token'] = token
                mail_template_values['ua'] = self.request.META['User-Agent']
                mail_template_values['ip'] = self.request.META['REMOTE_ADDR']
                path = 'mail/reset_password.txt'
...
class PasswordResetHandler(BaseHandler):
    def get(self, request, token):
        ...
        if q.count() == 1:
            ...
            path = 'desktop/reset_password.html'
            #output = template.render(path, template_values)
            #self.response.out.write(output)
            return render(request, path, template_values)
        else:
            path = 'desktop/token_not_found.html'
            #output = template.render(path, template_values)
            #self.response.out.write(output)
            return render(request, path, template_values)
    ...
    def post(self, request, token):
        ...
        if q.count() == 1:
            ...
            if errors == 0:
                ...
                prt.valid = 0
                prt.save()
                path = 'desktop/reset_password_ok.html'
                output = template.render(path, template_values)
                self.response.out.write(output)
            else:
                template_values['errors'] = errors
    ...
    def get(self, request, node_name):
        ...
        if node:
            ...
            if member:
                ...
                recent_nodes = cache.get('member::' + str(member.num) + '::recent_nodes')
                recent_nodes_ids = cache.get('member::' + str(member.num) + '::recent_nodes_ids')
                if recent_nodes and recent_nodes_ids:
                    if (node.num in recent_nodes_ids) is not True:
                        ...
                        cache.set('member::' + str(member.num) + '::recent_nodes', recent_nodes, 7200)
                        cache.set('member::' + str(member.num) + '::recent_nodes_ids', recent_nodes_ids, 7200)
                else:
                    recent_nodes = []
        ...
        if browser['ios']:
            if (node):
                path = 'mobile/node_graph.html'
            else:
                path = 'mobile/node_not_found.html'
        else:
            if (node):
                path = 'desktop/node_graph.html'
            else:
                path = 'desktop/node_not_found.html'
        #output = template.render(path, template_values)
        #self.response.out.write(output)
        return render(request, path, template_values)

class NodeHandler(View):
    def get(self, request, node_name):
        site = GetSite()
        browser = detect(self.request)
        self.session = Session()
        ...
        if node:
            ...
            q3 = Topic.objects.filter(node_num = node.num).order_by('-last_touched')[start:page_size]
            topics = q3
        template_values['latest'] = topics
        if browser['ios']:
            if (node):
                path = 'mobile/node.html'
            else:
                path = 'mobile/node_not_found.html'
        else:
            if (node):
                path = 'desktop/node.html'
            else:
                path = 'desktop/node_not_found.html'
        #output = template.render(path, template_values)
        #self.response.out.write(output)
        print (path ,template_values)
        return render(request, path ,template_values)

class NodeApiHandler(View):
    def get(self, request, node_name):
        site = GetSite()
        node = GetKindByName('Node', node_name)
        if node:
...
class SearchHandler(View):
    def get(self, request, q):
        ...
        if config.fts_enabled is not True:
            path = 'desktop/search_unavailable.html'
            #output = template.render(path, template_values)
            #self.response.out.write(output)
            return render(request, path, template_values)
        else:
            ...
            if topics is None:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        cache.set('q::' + q_md5, topics, 86400)
                except:
                    template_values['topics'] = []
            else:
                template_values['topics'] = topics
            path = 'desktop/search.html'
            #output = template.render(path, template_values)
            #self.response.out.write(output)
            return render(request, path, template_values)

class DispatcherHandler(View):
    def post(self, request):
...
class RouterHandler(View):
    def get(self, request, path):
        if path.find('/') != -1:
            ...
            # Site
            page = cache.get(path + '/index.html')
            if page:
                #taskqueue.add(url='/hit/page/' + str(page.key()))
                if page.mode == 1:
                    # Dynamic embedded page
                ...
                if q.count() == 1:
                    page = q[0]
                    cache.set(path + '/index.html', page, 864000)
                    if page.mode == 1:
                        # Dynamic embedded page
                        template_values = {}
                        site = GetSite()
...
class ChangesHandler(View):
    def get(self, request):
        ...
        try:
            page_current = int(self.request.GET.get('p'))
            if page_current < 1:
                page_current = 1
        ...
        latest = cache.get('q_changes_' + str(page_current))
        if (latest):
            template_values['latest'] = latest
        else:
            #q1 = db.GqlQuery("SELECT * FROM Topic ORDER BY last_touched DESC LIMIT " + str(page_start) + "," + str(page_size))
            q1 = Topic.objects.order_by('-last_touched').all()[page_start:page_start+page_size] # to be modify to right statement
            topics = []
            for topic in q1:
                topics.append(topic)
            cache.set('q_changes_' + str(page_current), topics, 120)
            template_values['latest'] = topics
            template_values['latest_total'] = len(topics)
        if browser['ios']:
            path = 'mobile/changes.html'
        else:
            path = 'desktop/changes.html'
        #output = template.render(path, template_values)
        #self.response.out.write(output)
        return render(request, path, template_values)
