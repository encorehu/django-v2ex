...
class NewTopicHandler(View):
    def get(self, request, node_name):
        ...
        if (member):
            ...
            if can_create:
                if browser['ios']:
                    if node:
                        path = 'mobile/new_topic.html'
                    else:
                        path = 'mobile/node_not_found.html'
                else:
                    if node:
                        path = 'desktop/new_topic.html'
                    else:
                        path = 'desktop/node_not_found.html'
            else:
                path = 'desktop/access_denied.html'
            #output = template.render(path, template_values)
            #self.response.out.write(output)
            return render(request, path, template_values)
        else:
    ...
    def post(self, request, node_name):
        ...
        if ('HTTP_HOST' in self.request.META):
            ...
            can_continue = False
        if ('HTTP_USER_AGENT' not in self.request.META):
            can_continue = False
        if ('HTTP_COOKIE' not in self.request.META):
            can_continue = False
        if ('HTTP_REFERER' in self.request.META):
            print '----in'
            has_v2ex = False
#            if ('http://localhost:10000' in self.request.META['HTTP_REFERER']):
#                has_v2ex = True
#            if ('http://www.v2ex.com' in self.request.META['HTTP_REFERER']):
#                has_v2ex = True
#            if ('http://v2ex.appspot.com' in self.request.META['HTTP_REFERER']):
#                has_v2ex = True
#            if ('https://www.v2ex.com' in self.request.META['HTTP_REFERER']):
#                has_v2ex = True
#            if ('http://jp.v2ex.com' in self.request.META['HTTP_REFERER']):
#                has_v2ex = True
#            if ('http://eu.v2ex.com' in self.request.META['HTTP_REFERER']):
#                has_v2ex = True
#            if ('http://us.v2ex.com' in self.request.META['HTTP_REFERER']):
#                has_v2ex = True
#            if ('https://v2ex.appspot.com' in self.request.META['HTTP_REFERER']):
#                has_v2ex = True
#            if ('http://fast.v2ex.com' in self.request.META['HTTP_REFERER']):
#                has_v2ex = True
...
#                has_v2ex = True
        ...
        if ('CONTENT_TYPE' in self.request.META):
            if self.request.META['CONTENT_TYPE'].startswith( 'application/x-www-form-urlencoded') is False:
                can_continue = False
        else:
            can_continue = False
        if can_continue is False:
            #return #self.redirect()
            return redirect('http://' + site.domain + '/')
        ### END: CAN CONTINUE
        browser = detect(self.request)
        template_values = {}
        template_values['site'] = site
        ...
        if (member):
            ...
            if can_create:
                ...
                topic_title = self.request.POST.get('title').strip().replace("\n", " ")
                ...
                topic_content = self.request.POST.get('content').strip()
                ...
                if site.use_topic_types:
                    types = site.topic_types.split("\n")
                    if len(types) > 0:
                        topic_type = self.request.POST.get('type','0').strip()
                        try:
                ...
                if (errors == 0):
                    ...
                    path = 'portion/topic_content.html'
                    output = template.loader.render_to_string(path, {'topic' : topic})
                    #topic.content_rendered = output.decode('utf-8')
                    topic.content_rendered = output.encode('utf-8')
                    topic.node = node
                    ...
                    cache.delete('Node_' + str(topic.node_num))
                    cache.delete('Node::' + str(node.name))
                    ...
                    if member.twitter_oauth == 1 and member.twitter_sync == 1:
                        ...
                        status = topic.title + ' #' + topic.node.name + ' http://' + self.request.META['HTTP_HOST'] + '/t/' + str(topic.num)
                        try:
                    ...
                    #self.redirect()
                    return redirect('/t/' + str(topic.num) + '#reply0')
                else:    
                    if browser['ios']:
                        path = 'mobile/new_topic.html'
                    else:
...
class TopicHandler(View):
    def get(self, request, topic_num):
        ...
        reply_reversed = self.request.GET.get('r')
        if reply_reversed == '1':
            reply_reversed = True
        else:
            reply_reversed = False
        filter_mode = self.request.GET.get('f')
        if filter_mode == '1':
            filter_mode = True
        else:
            filter_mode = False
        ...
        if len(topic_num_str) > 8:
            if browser['ios']:
                path = 'mobile/topic_not_found.html'
            else:
                path = 'desktop/topic_not_found.html'
            #output = template.render(path, template_values)
            #self.response.out.write(output)
            return render(request, path, template_values)
        topic = False
        topic = cache.get('Topic_' + str(topic_num))
        if topic is None:
            #q = db.GqlQuery("SELECT * FROM Topic WHERE num = :1", int(topic_num))
            q = Topic.objects.filter(num=int(topic_num))
            if (q.count() == 1):
                topic = q[0]
                cache.set('Topic_' + str(topic_num), topic, 86400)
        can_edit = False
        can_move = False
        if topic:
            if topic.content:
        ...
        if (topic):
            ...
            replies = False
            if browser['ios']:
                path = 'portion/topic_replies_mobile.html'
            else:
                path = 'portion/topic_replies.html'
            if filter_mode:
                if browser['ios']:
                ...
                r = cache.get(r_tag)
                if r is None:
                    replies = cache.get('topic_' + str(topic.num) + '_replies_filtered_compressed_' + str(page_current))
                    if replies is None:
                        ...
                        q5 = Reply.objects.filter(topic_num=topic.num,member_num=topic.member.num).order_by('created')[page_start:page_start+page_size]
                        replies = q5
                        cache.set('topic_' + str(topic.num) + '_replies_filtered_compressed_' + str(page_current), GetPacked(replies), 7200)
                    else:
                        replies = GetUnpacked(replies)
                    template_values['replies'] = replies
                    template_values['replies_count'] = replies.count()
                    r = template.loader.render_to_string(path, template_values)
                    cache.set(r_tag, r, 86400)
            else:    
                if reply_reversed:
                    if browser['ios']:
                    ...
                    r = cache.get(r_tag)
                    if r is None:
                        replies = cache.get('topic_' + str(topic.num) + '_replies_desc_compressed_' + str(page_current))
                        if replies is None:
                            #q4 = db.GqlQuery("SELECT * FROM Reply WHERE topic_num = :1 ORDER BY created DESC LIMIT " + str(page_start) + "," + str(page_size), topic.num)
                            q4 = Reply.objects.filter(topic_num=topic.num).order_by('-created')[page_start:page_start+page_size]
                            replies = q4
                            cache.set('topic_' + str(topic.num) + '_replies_desc_compressed_' + str(page_current), GetPacked(q4), 86400)
                        else:
                            replies = GetUnpacked(replies)
                        template_values['replies'] = replies
                        template_values['replies_count'] = replies.count()
                    ...
                    r = cache.get(r_tag)
                    if r is None:
                        replies = cache.get('topic_' + str(topic.num) + '_replies_asc_compressed_' + str(page_current))
                        if replies is None:
                            #q4 = db.GqlQuery("SELECT * FROM Reply WHERE topic_num = :1 ORDER BY created ASC LIMIT " + str(page_start) + "," + str(page_size), topic.num)
                            q4 = Reply.objects.filter(topic_num=topic.num).order_by('created')[page_start:page_start+page_size]
                            replies = q4
                            cache.set('topic_' + str(topic.num) + '_replies_asc_compressed_' + str(page_current), GetPacked(q4), 86400)
                        else:
                            replies = GetUnpacked(replies)
                        template_values['replies'] = replies
                        template_values['replies_count'] = replies.count()
    ...
    def post(self, request, topic_num):
        site = GetSite()
        ### BEGIN: CAN CONTINUE
        can_continue = True
        if ('HTTP_HOST' in self.request.META):
...
#            if ('http://beta.v2ex.com' in self.request.META['HTTP_REFERER']):
#                has_v2ex = True
            if ('http://' + site.domain in self.request.META['HTTP_REFERER']):
                has_v2ex = True
            if has_v2ex is False:
                can_continue = False
        else:
        ...
        if (member):
            ...
            reply_content = self.request.POST.get('content').strip()
            ...
            if (topic and (errors == 0)):
                ...
                cache.delete('topic_' + str(topic.num) + '_replies_desc_compressed_' + str(pages))
                cache.delete('topic_' + str(topic.num) + '_replies_asc_compressed_' + str(pages))
                cache.delete('topic_' + str(topic.num) + '_replies_filtered_compressed_' + str(pages))
                ...
                cache.delete('member::' + str(member.num) + '::participated')
                cache.delete('q_latest_16')
                cache.delete('home_rendered')
                cache.delete('home_rendered_mobile')
                if topic.replies < 50:
                ...
                if member.twitter_oauth == 1 and member.twitter_sync == 1:
                    ...
                    twitter = OAuthApi(CONSUMER_KEY, CONSUMER_SECRET, access_token)
                    if topic.replies > page_size:
                        link = 'http://' + self.request.META['HTTP_HOST'] + '/t/' + str(topic.num) + '?p=' + str(pages) + '#r' + str(topic.replies)
                    else:
                        link = 'http://' + self.request.META['HTTP_HOST'] + '/t/' + str(topic.num) + '#r' + str(topic.replies)
                ...
                if pages > 1:
                    #self.redirect()
                    return redirect('/t/' + str(topic.num) + '?p=' + str(pages) + '#reply' + str(topic.replies))
                else:
                    #self.redirect()
        ...
        if (member):
            if (can_edit):
                ...
                if (topic):
                    ...
                    q4 = Reply.objects.filter(topic_num = topic.num).order_by('created')
                    template_values['replies'] = q4
                    if browser['ios']:
                        path = 'mobile/edit_topic.html'
                    else:
                        path = 'desktop/edit_topic.html'
                else:
                    path = 'mobile/topic_not_found.html'
                #output = template.render(path, template_values)
                #self.response.out.write(output)
        ...
        if member:
            if can_edit:
                ...
                if (topic):
                    ...
                    topic_title = self.request.POST.get('title').strip()
                    if (len(topic_title) == 0):
                        errors = errors + 1
                        topic_title_error = 1
                    else:
                    ...
                    if site.use_topic_types:
                        types = site.topic_types.split("\n")
                        if len(types) > 0:
                            topic_type = self.request.POST.get('type').strip()
                            try:
                                topic_type = int(topic_type)
                                if topic_type < 0:
                                    topic_type = 0
        ...
        if member:
            if member.level == 0 and (str(member.created_ts) == str(t)):
                ...
                if q.count() == 1:
                    ...
                    q5 = TopicBookmark.objects.filter(topic=topic)
                    for bookmark in q5:
                        bookmark.delete()
                    # Take care of Node                
                    node = topic.node
...
class TopicPlainTextHandler(View):
    def get(self, request, topic_num):
        site = GetSite()
        ...
        if topic:
            template_values = {}
            template_values['topic'] = topic
            replies = cache.get('topic_' + str(topic_num) + '_replies_asc')
            if replies is None:
                #q = db.GqlQuery("SELECT * FROM Reply WHERE topic_num = :1 ORDER BY created ASC", topic.num)
                q = Reply.objects.filter(topic_num = topic.num ).order_by('created')
                replies = q
                cache.set('topic_' + str(topic_num) + '_replies_asc', q, 86400)
            if replies:
                template_values['replies'] = replies
            path = 'api/topic.txt'
            #output = template.render(path, template_values)
...
class ReplyEditHandler(View):
    ...
    def post(self, request, reply_num):
        template_values = {}
        site = GetSite()
        template_values['site'] = site
        member = CheckAuth(self)
