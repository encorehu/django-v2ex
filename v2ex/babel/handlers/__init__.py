...
class BaseHandler(View):
    ...
    def browser(self):
        if not hasattr(self, "_browser"):
            result = {}
            if 'User-Agent' in self.request.META:
                user_agent = self.request.META['User-Agent']
    ...
    def member(self):
        if not hasattr(self, "_member"):
            ...
            if 'X-Real-IP' in self.request.META:
                ip = self.request.META['X-Real-IP']
            else:
            ...
            if 'auth' in cookies:
                ...
                if member_num > 0:
                    ...
                    if member is None:
                        ...
                        if q.count() == 1:
                            member = q[0]
                            cache.set('Member_' + str(member_num), member, 86400 * 14)
                    if member:
                        member.ip = ip
                else:
                    ...
                    if (q.count() == 1):
                        member_num = q[0].num
                        member = q[0]
                        cache.set(auth, member_num, 86400 * 14)
                        cache.set('Member_' + str(member_num), member, 86400 * 14)
                        member.ip = ip
            self._member = member
        return self._member
    ...
    def finalize(self, template_name, mobile_optimized=False, template_root='desktop', template_type='html'):
        ...
        if self.browser['ios'] and mobile_optimized:
            path = 'mobile/'+template_name + '.html'
        else:
            path = template_root + '/' + template_name + '.' + template_type
        #output = template.render(path, self.values)
        #self.response.out.write(output)
        print '-'*80
        return render(self.request, path, self.values)
