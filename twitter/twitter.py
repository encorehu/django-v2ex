      ...
      if len(m) > 0:
        ...
        for s in m:
          if s != last:
            ...
            expanded = cache.get(cache_tag)
            if expanded is None:
              expanded = api.expand(s)
              cache.set(cache_tag, expanded, 2678400)
            last = s
...
class _MemCache(object):
  ...
  def Get(self, key):
    data = cache.get(self._GetCacheKey(key))
    if data is not None:
      return data[0]
    return None
  ...
  def Set(self, key, data):
    data = (data, time.time())
    cache.set(self._GetCacheKey(key), data)

  def Remove(self, key):
    cache.delete(self._GetCacheKey(key))
